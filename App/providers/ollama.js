class OllamaProvider {
  ...
  async _post(url, body, timeoutMs = 300000) {
    ...
    try {
      ...
      throw new Error('Ollama request failed: ' + (e.name === 'AbortError' ? 'timeout' : e.message));
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async sendMessage(messages, onChunk) {
    const response = await this._post(`${this.baseUrl}/api/chat`, {
      model: this.model,
    ...
    if (!response.body) throw new Error('Ollama API error: empty response body');
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          ...
          fullText += content;
          badLines = 0;
          if (fullText.length > 2000000) throw new Error('Ollama response too large (2MB cap)');
          onChunk(fullText);
        } catch (e) {
          if (e.message && e.message.includes('too large')) throw e;
          if (++badLines > 100) throw new Error('Ollama stream: too many malformed chunks');
        }
      }
    }
    return fullText;
