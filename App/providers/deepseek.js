class DeepSeekProvider {
  ...
  async _post(url, body, timeoutMs = 120000) {
    ...
    try {
      ...
      throw new Error('DeepSeek request failed: ' + (e.name === 'AbortError' ? 'timeout' : e.message));
    } finally {
      clearTimeout(timer);
    }
  }
  ...
  async sendMessage(messages, onChunk) {
    const response = await this._post('https://api.deepseek.com/v1/chat/completions', {
      model: this.model,
    ...
    if (!response.body) throw new Error('DeepSeek API error: empty response body');
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          fullText += content;
          badLines = 0;
          if (fullText.length > 2000000) throw new Error('DeepSeek response too large (2MB cap)');
          onChunk(fullText);
        } catch (e) {
          if (e.message && e.message.includes('too large')) throw e;
          if (++badLines > 100) throw new Error('DeepSeek stream: too many malformed chunks');
        }
      }
    }
    return fullText;
